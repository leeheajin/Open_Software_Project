		...
		for (int j = 0; j < col; j++) {
			...
			for (int a = -n; a <= n; a++) {
				for (int b = -n; b <= n; b++) {
					...
					sum_b += kernel.at<float>(n + a, n + b) * input.at<C>(tempa, tempb)[0];
					sum_g += kernel.at<float>(n + a, n + b) * input.at<C>(tempa, tempb)[1];
					sum_r += kernel.at<float>(n + a, n + b) * input.at<C>(tempa, tempb)[2];

				}

			}
			output.at<C>(i, j)[0] = 5 * abs(sum_b);
			output.at<C>(i, j)[1] = 5 * abs(sum_g);
			output.at<C>(i, j)[2] = 5 * abs(sum_r);
