		...
		for (int j = 0; j < col; j++) {
			...
			for (int a = -n; a <= n; a++) {
				for (int b = -n; b <= n; b++) {
					...
					sum_x += kernel_Sx.at<float>(n + a, n + b) * input.at<G>(tempa, tempb);
					sum_y += kernel_Sy.at<float>(n + a, n + b) * input.at<G>(tempa, tempb);

				}

			}
			output.at<G>(i, j) = sqrt(pow(sum_x, 2) + pow(sum_y, 2));
