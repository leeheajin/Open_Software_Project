...
//output is corner harris function.
//corner_mat = 1 for corner, 0 otherwise.
Mat NonMaximum_Suppression(const Mat input, Mat corner_mat, int radius)
{
	int row = input.rows;
	...
	for (int i = radius; i < row + radius; i++) {
		for (int j = radius; j < col + radius; j++) {
			int corner = 1;
			if (corner_mat.at<uchar>(i - radius, j - radius) != 0) { // If (i-radius, j-radius) is a corner now
			...
			if (corner == 0) { // (i-radius, j-radius) is not a corner now
